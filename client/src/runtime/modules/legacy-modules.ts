...
import type { RuntimeModule } from "../client-runtime";

const registerNavigationAutomation: RuntimeModule = ({ client }) => {
    const aliases = client.aliases;
    aliases.push({
        pattern: /\/fake (.*)/,
        callback: (matches: RegExpMatchArray) => {
            client.clientAdapter.output(
                client.clientAdapter.parseAnsiPatterns(client.onLine(matches[1], "combat.avatar")),
            );
            // @ts-ignore - legacy method requires flush
...
const registerTravelModules: RuntimeModule = ({ client }) => {
    const aliases = client.aliases;
...
const registerInventoryModules: RuntimeModule = ({ client }) => {
    const aliases = client.aliases;
...
const registerMagicAndEvaluationModules: RuntimeModule = ({ client }) => {
    const aliases = client.aliases;
...
const registerAlertAndUiModules: RuntimeModule = ({ client }) => {
    const aliases = client.aliases;
...
const registerPeopleAndGagsModule: RuntimeModule = ({ client }) => {
    new People(client);
    registerGagTriggers(client);
    registerLuaGagTriggers(client);
};

export function registerLegacyModules(runtime: ClientRuntime): void {
    runtime.registerModules([
        registerNavigationAutomation,
        registerTravelModules,
        registerInventoryModules,
        registerMagicAndEvaluationModules,
        registerAlertAndUiModules,
        registerPeopleAndGagsModule,
    ]);
}
