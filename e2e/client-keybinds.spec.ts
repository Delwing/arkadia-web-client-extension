...
async function getAllCommands(page: Page): Promise<string[]> {
    return await page.evaluate(() => [...((window as any).__mockCommandLog ?? [])]);
}
...
test.describe('Client keydown handler', () => {
    ...
    test.describe('Attack bind (Ctrl+1)', () => {
        ...
        test('sends "zabij ob_<id>" when attack target is present', async ({page}) => {
            ...
            await page.waitForFunction(() => localStorage.getItem('currentCharacter') === 'AttackHero');
    ...
    test.describe('Attack bind with custom attackCommand', () => {
        ...
        test('uses custom attack command after settings change', async ({page}) => {
            ...
            await page.waitForFunction(() => localStorage.getItem('currentCharacter') === 'CustomAttacker');
        ...
        test('uses default "zabij" when settings are cleared', async ({page}) => {
            ...
            await page.waitForFunction(() => localStorage.getItem('currentCharacter') === 'DefaultAttacker');
    ...
    test.describe('Support bind (Ctrl+Q)', () => {
        ...
        test('sends "wesprzyj" and "wesprzyj ob_<leaderId>" when in a team', async ({page}) => {
            ...
            await page.waitForFunction(() => localStorage.getItem('currentCharacter') === 'SupportHero');
            ...
            const commands = await getAllCommands(page);
            expect(commands).toContain('wesprzyj');
            expect(commands).toContain(`wesprzyj ob_${LEADER_ID}`);
...
test.describe('Client keydown handler › Custom binds from localStorage', () => {
    ...
    test('custom Ctrl+F6 bind does not fire without the Ctrl modifier', async ({page}) => {
        ...
        const commands = await page.evaluate(() => [...((window as any).__mockCommandLog ?? [])]);
        expect(commands).toHaveLength(0);
    });
