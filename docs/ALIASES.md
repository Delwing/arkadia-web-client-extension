...
> **Wskazowka:** W czasie jazdy `zerknij` zatrzymuje pojazd — wysyla `zatrzymaj woz` / `zatrzymaj bryczke` / `zatrzymaj dylizans`. Dziala tak samo klawisz (domyslnie Numpad5), jak i przycisk `zerknij` na panelu mobilnym czy desktopowym. Kiedy pojazd stoi, `zerknij` znowu rozglada sie po lokacji.
