...
> **Wskazowka:** Okno jest narzedziem diagnostycznym dla mechaniki zaslon. Gra nie wysyla stanu zaslony w GMCP - nie ma zadnego pola `covered_by` - wiec stan jest wnioskowany z linii tekstu i wygasa sam po 12 sekundach bez potwierdzenia. Gorna tabela pokazuje wszystkie postacie na lokacji podzielone na `Druzyna` i `Wrogowie`. Kluczowa jest kolumna **przed kim**: zaslona dziala tylko przeciw atakujacym wymienionym w linii `zaslania ... przed ciosami ...`, wiec ten sam cel moze byc zablokowany dla czlonka druzyny i calkowicie dostepny dla ciebie. Twoj wpis jest pogrubiony, a status `ZASLONIETY` (czerwony) oznacza, ze to *ty* nie mozesz trafic; `zaslaniany` (pomaranczowy) oznacza zaslone przed kims innym. Wpisy pisane kursywa sa tylko przypuszczeniem wywnioskowanym z GMCP (`attack_num`), nie z tekstu. Licznik `nieznane blokady` liczy sytuacje, w ktorych linia `staje ci na drodze` trafila na zaslone, o ktorej nie wiedzielismy - wartosc wieksza od zera w normalnej walce oznacza przeoczona linie zalozenia zaslony, nie szum. Wpis `WYGASLO` podaje przyczyne: `brak potwierdzenia` (TTL - 12 s bez zadnego potwierdzenia), `znikl z lokacji` (wraz z opisem, kto znikl), `smierc`, `ogluszenie` lub `limit wieku` (10 minut - zaslona nie ma wlasnego czasu trwania, wiec ten limit jest tylko zabezpieczeniem i w normalnej walce nie powinien wystapic). Dopoki zaslona trwa, GMCP samo ja potwierdza - `attack_num` zablokowanego atakujacego wskazuje zaslaniajacego, a zaslaniany nadal jest na lokacji - wiec TTL odlicza tylko wtedy, gdy stracimy ten slad. Dolny panel to log zdarzen (najnowsze na gorze, do 200 wpisow) z oryginalna linia gry pod kazdym wpisem; przycisk **bez GMCP** ukrywa wpisy wywnioskowane wylacznie z GMCP, **Wyczysc** czysci log i licznik. Udane `przelam obrone` zdejmuje zaslone dla calej druzyny naraz, nieudane nie zdejmuje jej nikomu.
