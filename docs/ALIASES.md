...
> **Wyrownanie tekstu:** linie tresci sa domyslnie justowane. Znak na poczatku linii zmienia jej wyrownanie (sam znak nie jest wysylany): `<` do lewej, `^` wysrodkowanie, `>` do prawej, `=` justowanie. Wyrownanie obejmuje cala linie, takze po zawinieciu. Przyciski nad polem tresci ustawiaja znacznik w liniach pod kursorem lub w zaznaczeniu. Aby zaczac linie jednym z tych znakow jako tekstem, poprzedz go `\`, np. `\>>`.
