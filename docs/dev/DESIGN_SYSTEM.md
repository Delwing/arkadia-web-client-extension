...
| `src/web/` knowledge + report popups (7) | **on the design system**, `--ark-*` only (Phase 3, PR 5). Czat is not among them — it is blocked by `.map-header-menu__*`, exactly as Okno mapy is; see `UI_MIGRATION.md` §4 |
