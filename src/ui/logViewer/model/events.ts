 ...
 *
 * `login` comes from the GMCP message type. The other two reuse lines the game
 * itself prints as markers, and whose patterns already exist in the client's
...
export const LOG_EVENT_KINDS = ["login", "death", "trait"] as const;
...
export const LOG_EVENT_META: Record<LogEventKind, LogEventMeta> = {
    ...
    trait: { label: "Awans cechy", tag: "AWANS", glyph: "▲", colorToken: "var(--ark-log-event-trait)" },
};
...
/** `Twoja sila osiagnela nadludzki poziom.` and its declensions. */
const TRAIT_LINE = /^Tw(?:oja|oj) [a-z]+ osiagn(?:ela|al) /;
...
export function detectEvent(text: string, type: string | undefined): LogEventKind | undefined {
    ...
    if (TRAIT_LINE.test(text)) return "trait";
    return undefined;
}
