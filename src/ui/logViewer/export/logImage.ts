...
export interface ImageLine {
    /** Rendered in the left gutter; omit for lines without a timestamp. */
    time?: string;
    /** The line's HTML, carrying the game's own colours. */
    html: string;
}
...
export interface ImageStyle {
    ...
    fontSize: number;
    fontFamily: string;
    /** Width of the text column in CSS pixels; drives where lines wrap. */
    containerWidth: number;
}

/** Renders to a PNG blob. Throws when the result would exceed the canvas cap. */
export async function renderLogImage(lines: ImageLine[], style: ImageStyle): Promise<Blob> {
    if (lines.length === 0) {
        throw new Error('Brak linii do zapisu');
    }

    const { bgColor, defaultColor, timeColor, fontSize, fontFamily, containerWidth } = style;
 ...
 *
 * Throws where the browser has no async clipboard or refuses images — Firefox
 * still does by default — so the caller can offer the download instead rather
