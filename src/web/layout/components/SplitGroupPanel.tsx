...
interface SplitGroupPanelProps {
  ...
  /** Panel ID hovered for a tab-stack drop. */
  stackTargetId?: string;
  /** Panel ID targeted for a within-split insert. */
  splitTargetId?: string;
  splitBefore?: boolean;
}

type SplitMember =
  | { kind: 'single'; id: string; panel: WindowRecord; splitFlex: number }
  | { kind: 'tabs'; id: string; panels: WindowRecord[]; activeId: string; splitFlex: number };

type DisplayItem =
  | SplitMember
  | { kind: 'preview'; id: string; splitTargetId: string; splitBefore: boolean };
...
export function SplitGroupPanel({
  ...
  splitTargetId,
  splitBefore,
}: SplitGroupPanelProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const crossVertical = side === 'top' || side === 'bottom';

  const sorted = [...panels].sort(
    (a, b) => (a.splitOrder ?? 0) - (b.splitOrder ?? 0)
  );

  // Group by splitOrder — tab-group members sharing same order render as one slot.
  ...
  // Build display list including the preview ghost.
  ...
  const handleSplitterDown =
    ...
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      e.preventDefault();
      const aboveEl = containerRef.current?.querySelector<HTMLElement>(
        `[data-split-panel="${aboveId}"]`
      );
      const belowEl = containerRef.current?.querySelector<HTMLElement>(
        `[data-split-panel="${belowId}"]`
      );
      if (!aboveEl || !belowEl) return;

      const startPos = crossVertical ? e.clientY : e.clientX;
      const startAboveSize = crossVertical
        ? aboveEl.offsetHeight
        : aboveEl.offsetWidth;
      const startBelowSize = crossVertical
        ? belowEl.offsetHeight
        : belowEl.offsetWidth;
  ...
  const splitterClass = `split-group-splitter split-group-splitter--${
    crossVertical ? 'h' : 'v'
  }`;
