...
/** Interfejs > Mapa. Migrated onto the design system (UI_MIGRATION.md §4). */
function MapSections({ draft, update, mapVersion, refreshing, onRefreshMap, explorationStats }: MapSectionsProps) {
    const refreshBadge = (
        <button type="button" id="ui-map-refresh-btn" className="settings-map-version" title="Kliknij, aby odświeżyć dane mapy" disabled={refreshing} onClick={onRefreshMap}>
            <span className="settings-map-version__icon">{'↻'}</span>
    ...
    return (
        <>
            <SettingsCard title="Mapa" headerExtra={refreshBadge}>
                <NumberField id="ui-map-scale" label="Powiększenie mapy" labelExtra={<DeviceOnlyBadge settingKey="mapScale" />} value={draft.mapScale} step={0.05} min={0.05} onChange={(n) => update({ mapScale: n })} />
                <NumberField id="ui-map-height" label="Wysokość mapy (vh)" labelExtra={<DeviceOnlyBadge settingKey="mapHeight" />} value={draft.mapHeight} step={1} onChange={(n) => update({ mapHeight: n })} />
                <SelectField id="ui-map-position" label="Położenie mapy" labelExtra={<DeviceOnlyBadge settingKey="mapPosition" />} value={draft.mapPosition} onChange={(v) => update({ mapPosition: v as UiSettings['mapPosition'] })}>
                ...
                <CheckboxField id="ui-instant-move" label="Natychmiastowe przechodzenie po mapie" checked={draft.instantMove} onChange={(v) => update({ instantMove: v })} />
                <CheckboxField id="ui-highlight-current-room" label="Podświetl bieżące pomieszczenie" checked={draft.highlightCurrentRoom} onChange={(v) => update({ highlightCurrentRoom: v })} />
                <CheckboxField
                    id="ui-exploration-mode"
                    label={<>Tryb eksploracji mapy <span id="ui-exploration-stats" className="settings-muted">{explorationStats}</span></>}
            ...
            <SettingsCard title="Pomieszczenia i linie">
            ...
            <SettingsCard title="Marker gracza">
                ...
                <CheckboxField id="ui-map-player-marker-dash-enabled" label="Przerywane obramowanie markera" checked={draft.mapPlayerMarkerDashEnabled} onChange={(v) => update({ mapPlayerMarkerDashEnabled: v })} />
                <MapPreviewCanvas
            ...
            <SettingsCard title="Podświetlenie pomieszczeń">
                <SettingsHint>Styl pierścieni rysowanych wokół wyróżnionych pomieszczeń (np. cel podróży, notatki). Kolor jest dobierany automatycznie dla każdego wyróżnienia.</SettingsHint>
                ...
                <CheckboxField id="ui-map-highlight-dash-enabled" label="Przerywane obramowanie" checked={draft.mapHighlightDashEnabled} onChange={(v) => update({ mapHighlightDashEnabled: v })} />
                <MapPreviewCanvas
