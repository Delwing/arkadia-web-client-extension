...
type Session struct {
	...
	*/
	...
	produced    int64
	deliveredTo int64
	// How far the next ping may confirm: where deliveredTo stood when the previous one
	// arrived. See confirmedLocked.
	confirmable int64
...
/*
confirmedLocked records that the client was alive just now, and lets go of what it has
certainly read — which is less than everything written before this moment.

A ping proves the page was running when it *sent* it, not when it arrived. Anything
written in between was still on the wire, or in the browser's network stack, when the
page produced that ping; if the tab froze and its socket died right after, those bytes
never reached the pipeline. Releasing them on arrival is what lost the tail of the
output — typically a prompt or a single line, a hundred-odd bytes — after even a brief
lock of a phone screen, reported on resume as "czesc tekstu przepadla".

So a ping releases only what had been written by the time the previous one arrived: the
client sent this one a full interval after that, long enough for those bytes to have
landed. Holding one interval longer costs nothing — a returning client names its own
offset, so a replay never repeats what it already has.
*/
func (s *Session) confirmedLocked() {
	s.releaseConfirmedLocked(s.confirmable)
	s.confirmable = s.deliveredTo
}

// releaseConfirmedLocked drops held writes that end at or before the given offset. A
// chunk straddling it stays whole: the replay trims it to the client's own offset.
func (s *Session) releaseConfirmedLocked(upTo int64) {
	i := 0
	for i < len(s.unconfirmed) {
		c := s.unconfirmed[i]
		if c.offset+int64(len(c.bytes)) > upTo {
			break
		}
		s.unconfirmedBytes -= len(c.bytes)
		i++
	}
	if i == len(s.unconfirmed) {
		s.unconfirmed = nil
		return
	}
	s.unconfirmed = s.unconfirmed[i:]
}
...
func (s *Session) attach(c clientConn, resumed bool, clientOffset int64) {
	...
	*/
	...
	s.droppedBytes = 0
	/*
		The replay is written to this client like anything else, so it is held like
		anything else. Letting it go here meant a phone whose fresh socket dropped again
		before the replay was read — flaky signal does exactly that — came back to find
		the replay gone and reported it as lost.

		Not for a session the game has ended: this attach drains and retires it, and
		there is nothing left to come back to.
	*/
	s.unconfirmed = nil
	s.unconfirmedBytes = 0
	if !s.closed {
		s.unconfirmed = pending
		s.unconfirmedBytes = replayed
		s.trimUnconfirmedLocked()
	}
	s.deliveredTo = s.produced
	// Nothing this client has been sent is confirmable until a ping has come and gone.
	s.confirmable = from
...
func (s *Session) write(b []byte) error {
	...
	if s.client != nil && !wasStale {
		// Proof the page is running, and so that what it was sent a ping ago was read.
		s.confirmedLocked()
	}
	if wasStale && s.client != nil {
		if len(s.pending) > 0 {
			s.flushPendingLocked()
		}
		// Nothing released on a wake: the page has only just started running again. The
		// next ping confirms whatever it has been handed by now.
		s.confirmable = s.deliveredTo
	}
	s.mu.Unlock()
	if closed || up == nil {
...
func (s *Session) finish(reason string) {
	...
	/*
		The client stays attached through the grace below, so its next pings can still
		confirm the last thing it was sent — two of them, since a ping only releases
		what had been written by the one before it (see confirmedLocked).
	...
	*/
	...
	if client != nil {
		...
		*/
		...
		// Held open just long enough for two more pings — the client sends them every
