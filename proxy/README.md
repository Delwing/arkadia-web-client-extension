...
A ping proves the page was running when it *sent* it, so it releases only what had been
written by the time the previous ping arrived. Releasing everything up to its own arrival
lost whatever was still in flight — typically a prompt, ~100 bytes — whenever a phone
locked its screen between the two. Replays are held the same way, so a resume whose fresh
socket drops again before reading them loses nothing either.
