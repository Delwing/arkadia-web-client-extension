...
// --- Keyboard movement --------------------------------------------------------
// Numpad-driven movement, mirroring the stock UI: we read the SAME shared
// `binds.directions` keymap the main profile uses (read-only — the alt UI never
// writes settings), so whatever numpad layout the user configured there works
// here unchanged, and edits made in the main UI re-apply live.
interface DirectionBind { key: string; ctrl?: boolean; alt?: boolean; shift?: boolean }

const DEFAULT_DIRECTION_BINDS: Record<string, DirectionBind> = {
    n: { key: 'Numpad8' }, s: { key: 'Numpad2' }, w: { key: 'Numpad4' }, e: { key: 'Numpad6' },
    nw: { key: 'Numpad7' }, ne: { key: 'Numpad9' }, sw: { key: 'Numpad1' }, se: { key: 'Numpad3' },
    u: { key: 'NumpadMultiply' }, d: { key: 'NumpadDivide' },
    zerknij: { key: 'Numpad5' }, special: { key: 'Numpad0' },
};

let directionBinds: Array<{ direction: string } & DirectionBind> = [];
function applyDirectionBinds(overrides?: Record<string, Partial<DirectionBind> | undefined>): void {
    directionBinds = Object.entries(DEFAULT_DIRECTION_BINDS).map(([direction, fallback]) => {
        const o = overrides?.[direction];
        const src = o?.key ? o : fallback;
        return { direction, key: src.key!, ctrl: !!src.ctrl, alt: !!src.alt, shift: !!src.shift };
    });
}
const readDirectionBinds = (binds?: { directions?: unknown }) =>
    applyDirectionBinds(binds?.directions as Record<string, DirectionBind> | undefined);
readDirectionBinds(globalStorage.get('binds'));
globalStorage.onChange('binds', readDirectionBinds);

document.addEventListener('keydown', (event) => {
    // Movement fires even while the command trough is focused (numpad is
    // dedicated to movement, as in the stock client), but never while some
    // other text field is being typed into.
    ...
    const bind = directionBinds.find(b =>
        event.code === b.key &&
        event.ctrlKey === !!b.ctrl && event.altKey === !!b.alt && event.shiftKey === !!b.shift);
    if (!bind) return;

    event.preventDefault();
    if (bind.direction === 'special') {
        const exits = (client.Map.currentRoom as { specialExits?: Record<string, unknown> } | undefined)?.specialExits ?? {};
        const first = Object.keys(exits)[0];
        if (first) void client.sendCommand(first);
        return;
    }
    void client.sendCommand(bind.direction);
});
